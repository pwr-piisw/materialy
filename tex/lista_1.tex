...
\title{PIISW, W08, IO, 2018/2019, semestr letni\\Lista zadań nr 1}
\author{mgr inż. Maciej Małecki\\ \small maciej.malecki@pwr.edu.pl
    \and mgr inż. Marcin Stachowiak\\ \small marcin.stachowiak@pwr.edu.pl}

\begin{document}
    \maketitle

    \section*{Wprowadzenie}
        Do realizacji zadań z~tej listy niezbędne jest wykonanie zadań z~listy~0, w~szczególności zaś konieczne jest posiadanie konta edukacyjnego na portalach \texttt{github.com} oraz \texttt{travis-ci.com}.
    ...
    \begin{enumerate}
        \item\label{exc:github-access}
            ...
            \begin{enumerate}
                ...
                \item Skonfiguruj lokalnego klienta GIT tak, aby dla domeny \texttt{github.com} użyty był wygenerowany klucz prywatny\footnote{Skorzystaj z~informacji zawartych w~\url{https://pwr-piisw.github.io/materialy/GIT\%20Cheat\%20Sheet.pdf}.}.
            \end{enumerate}

        \item\label{exc:git-repo}
            (2 pkt) Praca z lokalną kopią repozytorium.
            \begin{enumerate}
                \item Utwórz prywatne repozytorium w ramach organizacji \texttt{pwr-piisw}, nazwa repozytorium powinna być w~następującej formie: \texttt{imie\_nazwisko}.
        ...
        \item\label{exc:travis-ci}
            ...
            \begin{enumerate}
              \item Utwórz konto w~serwisie \texttt{travis-ci.com} oraz skojarz je ze swoim kontem edukacyjnym w~serwisie \texttt{github.com}.
              ...
              \item Zaktualizuj link do grafiki reprezentującej status \textit{builda} tak, aby wskazywał na właściwe środowisko w~ramach \texttt{travis-ci}.
            \end{enumerate}

        \item\label{exc:maven}
            (4 pkt) W~systemie obsługującym bibliotekę istnieją trzy komponenty: komponent zarządzający użytkownikami, komponent odpowiedzialny za książki oraz komponent w~którym znajduje się logika i~reguly biznesowe procesów wypożyczania książek. Komponent wypożyczania zalezy od komponentu obsługi książek, a~ten z~kolei zależy od komponentu obsługi użytkownika.
            \begin{enumerate}
